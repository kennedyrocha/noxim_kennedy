...
struct Channel: sc_module
{
  ...
  : sc_module(nm), targ_socket("targ_socket"), init_socket("init_socket")
  {
    ...
    cout << "data rate " << GlobalParams::channel_configuration[local_id].dataRate << " Gbps, transmission delay " << flit_transmission_delay_ps << " ps, " << flit_transmission_cycles << " cycles " << endl; 
