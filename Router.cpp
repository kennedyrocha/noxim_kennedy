      ...
      for (int j = 0; j < DIRECTIONS + 2; j++)
    {
      ...
      if (!buffer[i].IsEmpty())
      {
          ...
          if (current_level_tx[o] == ack_tx[o].read())
          {
              ...
              //~ Escreve em cada flit qual será seu next hop
              flit.nexthop = -1;
              if(o != DIRECTION_LOCAL){
                  int nexthop = -1;
                  AdmissibleOutputs outputs = routing_table.getAdmissibleOutputs(i, flit.dst_id);
                  ...
                  //~ if(flit.flit_type == FLIT_TYPE_HEAD)
                      //~ cout << "\t- O próximo pulo é o roteador " << nexthop << endl;
              }
                  ...
                  max_volume_to_be_drained)
              {
                  if (drained_volume >= GlobalParams:: max_volume_to_be_drained) {
                    cout << "Dying because drained_volume >= GlobalParams:: max_volume_to_be_drained" << endl;
                    sc_stop(); //~ this kill simulation with very huge workload. [input trace]
                  }
                  else
                  {
                  drained_volume++;
...
vector < int > Router::routingFunction(const RouteData & route_data)
{
    if (GlobalParams::use_winoc)
    {
          ...
          int nexthop = -1;
          AdmissibleOutputs outputs = routing_table.getAdmissibleOutputs(route_data.dir_in, route_data.dst_id);
          for (AdmissibleOutputs::iterator it = outputs.begin(); it != outputs.end(); it++){
            const LinkId & out_link = *it;
            nexthop = out_link.second;
          }
...
int Router::route(const RouteData & route_data)
{
    ...
    //~ cout << endl << "\t- Sou " << local_id << " e buscarei uma rota." << endl;
...
unsigned long long int Router::getRoutedFlits()
{
    return routed_flits;
}
