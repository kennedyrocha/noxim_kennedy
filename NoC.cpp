...
void NoC::buildMesh()
{
    ...
    // traffic trace - transmitted, eof and MPI
    GlobalParams::proctime_step_forward = 0;
    GlobalParams::trace_eof = new bool  [GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
    GlobalParams::finalized = new unsigned long long int *  [GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
    GlobalParams::blocked = new bool  [GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
    GlobalParams::hasBarrier = new bool  [GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
    GlobalParams::proctime = new unsigned long long int [GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
    GlobalParams::packet_queue = new unsigned long long int [GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
    GlobalParams::flits_queue = new unsigned long long int [GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];

    GlobalParams::barrier = 0;
    for (int i = 0; i < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y; i++) {
        GlobalParams::trace_eof[i]      = false;
        GlobalParams::finalized[i]      = new unsigned long long int[GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y];
        GlobalParams::blocked[i]        = false;
        GlobalParams::hasBarrier[i]     = false;
        GlobalParams::proctime[i]       = 0;
        GlobalParams::packet_queue[i]   = 0;
        GlobalParams::flits_queue[i]    = 0;
    }
    ...
    for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
        ...
        t[i][j]->pe->traffic_trace = new GlobalTrafficTrace(GlobalParams::traffic_trace_filename.c_str(), t[i][j]->pe->local_id);
