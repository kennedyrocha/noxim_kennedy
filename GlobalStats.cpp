...
double GlobalStats::getAverageDelay()
{
    ...
    for (int x = 0; x < GlobalParams::mesh_dim_x; x++) {
        unsigned long long int received_packets =
...
{
    unsigned long long int total_cycles = GlobalParams::simulation_time - GlobalParams::stats_warm_up_time;

    return (double)getReceivedFlits()/(double)(total_cycles);
}

unsigned long long int GlobalStats::getReceivedPackets()
{
...
unsigned long long int GlobalStats::getReceivedFlits()
{
    unsigned long long int n = 0;
...
vector < vector < unsigned long long int > > GlobalStats::getRoutedFlitsMtx()
{

    vector < vector < unsigned long long int > > mtx;
...
unsigned long long int GlobalStats::getWirelessPackets()
{
...
void GlobalStats::showStats(std::ostream & out, bool detailed)
{
    ...
    */
    ...
    vector < vector < unsigned long long int > > rf_mtx = getRoutedFlitsMtx();
...
*/
    ...
    unsigned long long int total_cycles =  sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
    unsigned long long int total_hpp = GlobalParams::total_headtail+GlobalParams::total_payload+GlobalParams::total_padding;
    unsigned long long int total_flits = ceil(ceil(total_hpp/(double) getReceivedPackets())/((double)GlobalParams::flit_size/8))*getReceivedPackets();

    out <<endl;
    out << "% ** Data transmited **\tbyte" << endl;
    out << "# Total headtail:\t" << (GlobalParams::total_headtail) << endl;
    out << "# Total payload:\t" << (GlobalParams::total_payload) << endl;
    out << "# Total padding:\t" << (GlobalParams::total_padding) << endl;
    out << "% Total h+p+p:\t\t" << total_hpp << endl;
    out << "%" << endl;
    out << "# Total transmit_flits:   " << total_flits << endl;
/*
	o Método getReceivedPackets do noxim está incorreto. Ele busca a quantidade de mensagens (total de linhas do arquivo, e não considera
	o valor máximo e mínimo dos pacotes, que causam overhead na geração de novos pacotes ao atingir seu tamanho maximo).
	Para obter o valor real dos flits transmitidos deve-se contar na transmissão em uma variavel global e imprimir aqui - já está feito
	mas não mandei imprimir aqui para a simulação até o momento, percebi agora dia 03/06/2018 as 01h27.
	Outra forma de obter este valor é calculando =ARREDONDAR.PARA.CIMA(V4/1500;0)*U4 onde 1500 é o tamanho maximo do pacote ENoC, neste exemplo.
	Consequentemente a quantidade de flits também mudará. Basta calcular corretamente o numero de pacotes e dividir por seu tamanho em Bytes.
*/
    ...
    out << "# Total executed_cycles+sf:  " << (total_cycles + GlobalParams::proctime_step_forward) << endl;
    out << "# Total step_forward:  " << GlobalParams::proctime_step_forward << endl;
    out << "% Total executed_time:    " << ceil(total_cycles/GlobalParams::clock_period_ps)  << " ns" << endl;
    ...
    out << "% Total bits over wireless:    " << GlobalParams::total_wirelessflits * GlobalParams::flit_size << endl;
    out <<endl;
...
void GlobalStats::showPowerManagerStats(std::ostream & out)
{
    ...
    unsigned long long int total_cycles = sc_time_stamp().to_double() / GlobalParams::clock_period_ps - GlobalParams::reset_time;
