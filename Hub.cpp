    ...
    */
	...
	if (!buffer_from_tile[i].IsEmpty()) 
	{
		...
		assert(r_from_tile[i]==DIRECTION_WIRELESS);
	          ...
	          /*
	           *  Ao enviar por wireless, a função tile2Hub
	           *  verifica se o destino final está conectado.
	           *  A alteração é verificar se o próximo pulo está conectado.
	           */
                //~ int channel = selectChannel(local_id,tile2Hub(flit.dst_id)); // ORIGINAL
                int channel = selectChannel(local_id,tile2Hub(flit.nexthop)); // ALTERADO

                //~ cout << "\t- Enviando pelo HUB => flit.dst_id=" << flit.dst_id << " flit.nexthop=" << flit.nexthop << " Channel=" << channel << endl;
                //~ cout <<  "\tHubData:" << "\t- Hub_id=" << local_id <<  "\t- " << flit <<  "\t- nexthop=" << flit.nexthop << "\t- channel=" << channel << endl;
	...
	{     
		...
		if (!(init[channel]->buffer_tx.IsFull()) )
		{
			...
			tile2antenna_reservation_table.release(channel);
                    ...
                    //cout << "\t- O flit " << flit << " já fo para o buffer_tx" << endl;
