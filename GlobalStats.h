...
class GlobalStats {
  ...
  public:
    ...
    unsigned long long int getReceivedPackets();

    // Returns the total number of received flits
    unsigned long long int getReceivedFlits();

    // number of packets that used the wireless network
    unsigned long long int getWirelessPackets();


    // Returns the number of routed flits for each router
     vector < vector < unsigned long long int > > getRoutedFlitsMtx();
...
#ifdef TESTING
    unsigned long long int drained_total;
#endif
